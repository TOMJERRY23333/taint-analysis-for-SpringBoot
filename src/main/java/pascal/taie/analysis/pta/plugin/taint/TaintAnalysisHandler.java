...
public class TaintAnalysisHandler extends OnFlyHandler {
    ...
    * */
    ...
    public boolean judgeCheck(Edge<CSCallSite, CSMethod> edge,CSVar csVar) {
        ...
        // 3. 当已确认args或var存在污点,
        // 处理不同对象类型的变量之间的污点传播,那么返回导致var或return(lhs)存在污点,
...
//        获取被调用方法的污点转移集合 tfs
        ...
        // 3. 判断是否为Callsource,是则传播<lhs,taint>,使得pts<lhs>包含taint,无需探究深入函数内部
        // 这是原来TaintAnalysisHandler插件的onNewCallEdge方法
...
//                                param, new SourceInfo(indexRef, taint));
        ...
        boolean taintOfFieldFlag = fieldContainers.contains(method.getRef().getDeclaringClass());

        if(taintOfVarFlag || taintOfArgFlag || taintOfParamFlag || taintOfFieldFlag){
            return true;
        }
        return false;
    }

    private Set<JClass> fieldContainers(Map<JField,Type> fieldSources) {

        Set<JClass> fieldContainerList = new HashSet<>();
        for (JField field : fieldSources.keySet()) {
            fieldContainerList.add(field.getDeclaringClass());
        }
