...
public class CreateEdge {
    ...
    public void initCallGraph(String configPath) {
        try {
            ...
            // Scan the entire project, get and classify all beans
            // 算法2:包含了将advice方法织入目标方法的方法体jimple的操作
    ...
    private void scanAllBeans(String configPath) {
        ...
        if (bean_xml_paths.size() > 0) {
            // XmlBeanClazz是一个静态类，包括sootclass类和字符串scope（单例和原型），从xml文件中读取的
        ...
        // gsc为合成类对象实例,目的是为没有具体类实现的接口或抽象类生成合成类
        ...
        // iterator为该程序的类的集合的迭代器
        while (iterator.hasNext()) {

            SootClass sootClass = iterator.next();
            if(sootClass.getName().equals("org.springframework.boot.autoconfigure.mail.MailSenderJndiConfiguration"))
                sootClass.getName();
            // 通过annotationAnalysis实例获取当前sootclass所有@bean注解的方法
            allBeans.addAll(annotationAnalysis.getAllBeans(sootClass));
//            elementsUnsorted.stream().filter(sootclass-> "org.springframework.boot.autoconfigure.mail.MailSenderJndiConfiguration".equals(sootclass.getName()));
            int annotationTag = annotationAnalysis.getAllComponents(sootClass);
            // 判断当前sootclass是否为bean
            if (SpringAnnotationTag.isBean(annotationTag)) {
                SootClass implClass = sootClass;
                // 是则加入allBeansAndInterfaces
                ...
                // 将接口名映射到对应的SootClass类，以及将SootClass类名映射到对应的SootClass类。
        ...
        allBeans.forEach(m->{if(m.getName().equals("com.favorites.service.ConfigService")) {
            m.getName();}
        });
        //建模数据库模型sootclass类
        Iterator<SootClass> iteratorMapper = elementsUnsorted.iterator();
        Hierarchy hierarchy = new Hierarchy();
        //databaseModels 存储数据库模型的SootClass类的集合
    ...
    private void linkMainAndController(SootClass controller, SootMethod psm) {
        ...
        String methodName;
//        if(sootClass.getName().equals("org.springframework.web.bind.annotation.RestController"))
//            sootClass.getName();
        try {
